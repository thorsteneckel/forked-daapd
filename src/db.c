...
#define U_V1600_CREATE_TBL_FILELIST			\
    "CREATE TABLE IF NOT EXISTS filelist ("		\
    "   path        VARCHAR(4096) PRIMARY KEY NOT NULL,"\
    "   name        VARCHAR(255) NOT NULL,"		\
    "   type        INTEGER NOT NULL,"			\
...
#define U_V1600_ALTER_TBL_FILES_ADD_COL			\
  "ALTER TABLE files ADD COLUMN virtual_path VARCHAR(4096) DEFAULT NULL;"

#define U_V1600_ALTER_TBL_PL_ADD_COL			\
...
static const struct db_init_query db_upgrade_v1600_queries[] =
  {
    { U_V1600_CREATE_TBL_FILELIST, "create new table filelist" },
    { U_V1600_ALTER_TBL_FILES_ADD_COL, "alter table files add column virtual_path" },
    { U_V1600_ALTER_TBL_PL_ADD_COL,    "alter table playlists add column virtual_path" },
...
static int
db_upgrade_v1600_filelist_add(const char *virtual_path, int type)
{
  char path[PATH_MAX];
  char parentpath[PATH_MAX];
  char *name;
...
db_upgrade_v1600(void)
{
  ...
  while ((ret = sqlite3_step(stmt)) == SQLITE_ROW)
      ...
      if (ret != SQLITE_OK)
	{
	  ...
	  || data_kind == 2 /* Spotify */)
	{
	  ret = db_upgrade_v1600_filelist_add(virtual_path, 3); /* Real file, spotify url, http url */
	  if (ret < 0)
	    {
  ...
  while ((ret = sqlite3_step(stmt)) == SQLITE_ROW)
      ...
      if (data_kind == 0) /* Excludes default playlists */
	{
	  ...
	  ret = db_upgrade_v1600_filelist_add(virtual_path, 1); /* Playlists */
	  if (ret < 0)
	    {
...
#define Q_VACUUM "VACUUM;"
  ...
  if (db_ver_major < 10)
      ...
      switch (db_ver)
	{
	  ...
	  case 1501:
	    ret = db_generic_upgrade(db_upgrade_v1600_queries, sizeof(db_upgrade_v1600_queries) / sizeof(db_upgrade_v1600_queries[0]));
	    if (ret < 0)
	      return -1;

	    ret = db_upgrade_v1600();
	    if (ret < 0)
	      return -1;
